...
const DestinosInternacionales = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      try {
        ...
        console.log('DestinosInternacionales - initial query result', { data, error });

        if (!error && data && data.length > 0) {
          setInternationalPackages(data);
        } else {
          if (error) console.warn('DestinosInternacionales - query error:', error);
          ...
          try {
            const { data: allData, error: allError } = await supabase
              ...
              .select('*, destination:destinations!inner(*)')
              .order('created_at', { ascending: false });
            console.log('DestinosInternacionales - fallback query result', { allData, allError });
            if (!allError && allData) {
              setInternationalPackages(allData.filter((pkg) => pkg.destination?.category === 'international'));
            }
          } catch (fallbackErr) {
            console.error('DestinosInternacionales - fallback error:', fallbackErr);
          }
        }
      } catch (err) {
        console.error('Error fetching international plans:', err);
      }
      setLoading(false);
    };
    ...
    const channel = supabase.channel('international-plans-changes')
  ...
  const isFiltering = searchTerm.trim().length > 0 || selectedDays != null || maxPrice !== 5000000;
  ...
  return (
    <section id="destinos-internacionales" className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <h2 className="text-4xl font-bold text-center mb-8 text-gray-800">Destinos Internacionales</h2>
