...
const DestinosInternacionales = () => {
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          ...
          try {
            ...
            if (!allError && allData) setInternationalPackages(allData);
          } catch (fallbackErr) {
  ...
  const maxIndex = Math.max(0, internationalPackages.length - itemsPerPage);
  ...
  return (
    <section id="destinos-internacionales" className="py-16 bg-muted/30">
          ...
          <div className="relative">
            <div className="overflow-hidden">
              <AnimatePresence mode="wait">
                ...
                >
                  {internationalPackages.slice(currentIndex, currentIndex + itemsPerPage).map((pkg) => (
                    <PackageCard
                      key={pkg.id}
                      package={{
                        id: pkg.id,
